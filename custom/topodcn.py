...
class ClosTopo(Topo):
    ...
    def buildCluster(self, name: str, noSpines: int, noAggrs: int):
        ...
        # return switches for external link
        return ext_switchs
    ...
    def addDFD(self, name: str, noClsts: int):
        # add datacenter level freedom border
        self.DFDs[name] = []
        for i in range(4):
    ...
    """
    # noDFDs: number of datacenter freedoms
    """
    def buildCampus(self, noDFDs: int):

        # add campus freedom borders
        self.CFDBs = []
        for i in range(4):
            CFDB = self.addSwitch(f'CFDB{i}')
            self.CFDBs.append(CFDB)

        # add datacenter freedoms (DFDs)
        self.DFDs = {}
